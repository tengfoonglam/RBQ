...
gnome-terminal --tab --title="Motion"   -- bash -i -c "bash scripts/start_motion.bash --sim ${ROBOT_ARGS[@]} ${IFACE_ARGS[@]}"
sleep 1
if [ "$VISION_ENABLED" = "true" ]; then
    ...
    gnome-terminal --tab --title="Mujoco" -- bash -i -c "bash scripts/start_mujoco.bash ${ROBOT_ARGS[@]} --vision ${IFACE_ARGS[@]}"
else
    gnome-terminal --tab --title="Mujoco" -- bash -i -c "bash scripts/start_mujoco.bash ${ROBOT_ARGS[@]} ${IFACE_ARGS[@]}"
fi

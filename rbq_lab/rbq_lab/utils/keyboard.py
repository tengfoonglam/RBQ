...
class KeyboardController:
    ...
    def update_commands(self, env):
        # for j in range(3):
        #     if self.velocity_commands[j] > 0:
        #         self.velocity_commands[j] = max(0, self.velocity_commands[j] * 0.90)
        #     elif self.velocity_commands[j] < 0:
        #         self.velocity_commands[j] = min(0, self.velocity_commands[j] * 0.90)
        ...
        if self.velocity_commands[1] >= 0:
            ...
            # self.velocity_commands[1] = max(-0.5, self.velocity_commands[1])
        ...
        for j in range(3):
            env.unwrapped.commands[:, j] = self.velocity_commands[j]

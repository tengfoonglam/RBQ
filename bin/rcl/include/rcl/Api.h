...
public:
    ...
    struct Joint {
        ...
        static constexpr float TORQUE_WHEEL_UPPER = 30.0f;   ///< @brief Upper bound torque for wheel joints [Nm] @ingroup JointControlAPI
        static constexpr float TORQUE_WHEEL_LOWER = -30.0f;  ///< @brief Lower bound torque for wheel joints [Nm] @ingroup JointControlAPI
    ...
    struct ArmJoint {
        /**
         * @defgroup ArmJointControlAPI Arm Joint Control API
         * @brief APIs for controlling arm/manipulator joint-level commands such as position, torque, and gains.
         *
         * This group includes functions to set arm joint position references, torque references,
         * and control gains (Kp and Kd) for each arm joint. These APIs are designed to support
         * real-time joint control in a multi-process system architecture.
         *
         * ### Motion Owner
         * Although there is only one physical robot, multiple processes may exist that generate reference commands.
        ...
        ArmJoint(RBQ_API* parent) : m_parent(parent) {}

        /**
         * @ingroup ArmJointControlAPI
         * @brief Enumeration of all arm joint IDs.
         *
         * The manipulator has 6 arm joints and 1 gripper joint.
         * This enum provides symbolic names for each joint, corresponding to their index.
         */
        enum class ArmJointID : int {
            J0 = 0,  ///< Arm joint 0
            J1 = 1,  ///< Arm joint 1
            J2 = 2,  ///< Arm joint 2
            J3 = 3,  ///< Arm joint 3
            J4 = 4,  ///< Arm joint 4
            J5 = 5,  ///< Arm joint 5
            GRIPPER = 6,  ///< Gripper joint
        };

        /**
         * @brief Set all arm joint references from local to the robot.
         *
         * This function synchronizes the all arm joint reference values (position, torque, Kp, Kd)
         * from the local variable within this API instance to the robot.
         * It should be called periodically once after setting each joint reference values.
         * Such as at the end of the each control loop iteration.
         *
         * @return Returns 1 on successful update.
        ...
        /**
         * @brief Assigns motion ownership of the specified arm joint to the current process.
         *
         * In a multi-process control architecture, multiple processes may generate reference commands.
         * However, only the process that owns a joint (the motion owner) can actively control it.
         *
         * This function assigns the current process as the motion owner of the specified arm joint.
         * When ownership is assigned, the existing reference values (position, torque, Kp, Kd)
         * are initialized using the values from the default system process.
         *
         * This function must be called once before using any reference-setting functions
         ...
         * If this process is not the motion owner, any reference it sets will be ignored by the controller.
         *
         * @param _jointId The arm joint ID (valid range: 0 to 6).
         *
         * @return Returns 1 if ownership is successfully assigned;
         *         -2 if _jointId is out of range.
         *
        ...
        /**
         * @brief Overload using ArmJointID.
         * @see setMotionOwner(const int&)
         * @ingroup ArmJointControlAPI
         */
        int setMotionOwner(const ArmJointID _jointId){
            return setMotionOwner(static_cast<int>(_jointId));
        }

        /**
         * @brief Sets the arm joint position reference.
         *
         * This function sets the desired joint position (in radians) for the specified arm joint.
         *
         * @param _jointId The arm joint ID (valid range: 0 to 6).
         * @param _joint_pos_rad The desired joint position in radians.
         *
         ...
         * @ingroup ArmJointControlAPI
         */
        int setPosRef(const ArmJointID _jointId, const float &_joint_pos_rad){
            return setPosRef(static_cast<int>(_jointId), _joint_pos_rad);
        }

        /**
         * @brief Sets the arm joint torque reference in Newton-meters (Nm).
         *
         * This function sets the desired torque value for a specific arm joint.
         *
         * @param _jointId The ID of the arm joint (valid range: 0 to 6).
         * @param _torque_Nm The desired torque in Newton-meters (Nm).
         *
         * @return Returns:
         * -  1 if the torque is set and the current process is the motion owner.
         ...
         * @ingroup ArmJointControlAPI
         */
        int setTorqueRef(const ArmJointID _jointId, const float &_torque_Nm){
            return setTorqueRef(static_cast<int>(_jointId), _torque_Nm);
        }

        /**
         * @brief Sets the arm joint position gain (Kp) with quantization.
         *
         * This function sets the desired Kp gain for a specified arm joint after quantizing it based on predefined resolution steps.
         *
         * @param _jointId The ID of the arm joint (valid range: 0 to 6).
         * @param _set_kp_ The desired Kp gain value in Nm/rad (valid range: 0.0 to 5000.0).
         * @param real_kp_ Reference parameter to receive the actual applied gain value after quantization.
         ...
         * @ingroup ArmJointControlAPI
         */
        int setGainKpRef(const ArmJointID _jointId, const float &_set_kp, float &real_kp_){
            return setGainKpRef(static_cast<int>(_jointId), _set_kp, real_kp_);
        }
        ...
        /**
         * @brief Overload using ArmJointID, and without return gain value.
         * @see setGainKpRef(const int&, const float&, float&)
         * @ingroup ArmJointControlAPI
         */
        int setGainKpRef(const ArmJointID _jointId, const float &_set_kp){
            float dummy;
            return setGainKpRef(static_cast<int>(_jointId), _set_kp, dummy);
        }

        /**
         * @brief Sets the arm joint damping gain (Kd) with quantization.
         *
         * This function sets the desired Kd gain for a specified arm joint after quantizing it based on predefined resolution steps.
         *
         * @param _jointId The ID of the arm joint (valid range: 0 to 6).
         * @param _set_kd The desired Kd gain value in Nm*s/rad (valid range: 0.0 to 10.0).
         * @param real_kd_ Reference parameter to receive the actual applied damping gain value after quantization.
         ...
         * @ingroup ArmJointControlAPI
         */
        int setGainKdRef(const ArmJointID _jointId, const float &_set_kd, float &real_kd_){
            return setGainKdRef(static_cast<int>(_jointId), _set_kd, real_kd_);
        }
        ...
        /**
         * @brief Overload using ArmJointID, without return gain value.
         * @see setGainKdRef(const int&, const float&, float&)
         * @ingroup ArmJointControlAPI
         */
        int setGainKdRef(const ArmJointID _jointId, const float &_set_kd) {
            float dummy;
            return setGainKdRef(static_cast<int>(_jointId), _set_kd, dummy);
        }

         /**
         * @brief Returns the current arm joint position in radians.
         *
         * Retrieves the measured joint angle for the specified arm joint.
         * The returned value is valid only when result == 1.
         *
         * @param _jointId Arm joint ID (valid: 0–6).
         * @param result Set to 1 on success, -1 if data is unavailable, -2 if joint ID is out of range.
         * @return Joint position [rad].
         *
         * @ingroup ArmJointControlAPI
         ...
         * @ingroup ArmJointControlAPI
         */
        float getPos(const ArmJointID _jointId, int &result){
            return getPos(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the joint position.
         * Use getPos(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getPos(const ArmJointID _jointId){
            int dummy;
            return getPos(static_cast<int>(_jointId), dummy);
        }

         /**
         * @brief Returns the current arm joint velocity in radians per second.
         *
         * Retrieves the measured joint angular velocity for the specified arm joint.
         * The returned value is valid only when result == 1.
         *
         * @param _jointId Arm joint ID (valid: 0–6).
         * @param result Set to 1 on success, -1 if data is unavailable, -2 if joint ID is out of range.
         ...
         * @ingroup ArmJointControlAPI
         */
        float getVel(const ArmJointID _jointId, int &result){
            return getVel(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the joint velocity.
         * Use getVel(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getVel(const ArmJointID _jointId){
            int dummy;
            return getVel(static_cast<int>(_jointId), dummy);
        }

         /**
         * @brief Returns the current arm joint torque in Newton-meters (Nm).
         *
         * Retrieves the measured torque applied to the specified arm joint.
         * The returned value is valid only when result == 1.
         *
         * @param _jointId Arm joint ID (valid: 0–6).
         * @param result Set to 1 on success, -1 if data is unavailable, -2 if joint ID is out of range.
         ...
         * @ingroup ArmJointControlAPI
         */
        float getTorque(const ArmJointID _jointId, int &result){
            return getTorque(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the joint torque.
         * Use getTorque(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getTorque(const ArmJointID _jointId){
            int dummy;
            return getTorque(static_cast<int>(_jointId), dummy);
        }

         /**
         * @brief Returns the current arm joint position gain (Kp) in Nm/rad.
         *
         * The returned value is valid only when result == 1.
         *
         * @param _jointId Arm joint ID (valid: 0–6).
         ...
         * @ingroup ArmJointControlAPI
         */
        float getGainKp(const ArmJointID _jointId, int &result){
            return getGainKp(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only Kp.
         * Use getGainKp(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getGainKp(const ArmJointID _jointId){
            int dummy;
            return getGainKp(static_cast<int>(_jointId), dummy);
        }

         /**
         * @brief Returns the current arm joint damping gain (Kd) in Nm*s/rad.
         *
         * The returned value is valid only when result == 1.
         *
         * @param _jointId Arm joint ID (valid: 0–6).
         ...
         * @ingroup ArmJointControlAPI
         */
        float getGainKd(const ArmJointID _jointId, int &result){
            return getGainKd(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only Kd.
         * Use getGainKd(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getGainKd(const ArmJointID _jointId){
            int dummy;
            return getGainKd(static_cast<int>(_jointId), dummy);
        }

         /**
         * @brief Returns the current position reference for the specified arm joint.
         *
         * Retrieves the position reference (setpoint) in radians.
         * The returned value is valid only when result == 1.
         *
         ...
         * @ingroup ArmJointControlAPI
         */
        float getPosRef(const ArmJointID &_jointId, int &result){
            return getPosRef(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the position reference.
         * Use getPosRef(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getPosRef(const ArmJointID &_jointId){
            int dummy;
            return getPosRef(_jointId, dummy);
        }

         /**
         * @brief Returns the current torque reference for the specified arm joint.
         *
         * Retrieves the torque setpoint in Newton-meters.
         * The returned value is valid only when result == 1.
         *
         ...
         * @ingroup ArmJointControlAPI
         */
        float getTorqueRef(const ArmJointID &_jointId, int &result){
            return getTorqueRef(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the torque reference.
         * Use getTorqueRef(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getTorqueRef(const ArmJointID &_jointId){
            int dummy;
            return getTorqueRef(_jointId, dummy);
        }

         /**
         * @brief Returns the current position gain (Kp) reference for the specified arm joint.
         *
         * Retrieves the Kp setpoint in Nm/rad.
         * The returned value is valid only when result == 1.
         *
         ...
         * @ingroup ArmJointControlAPI
         */
        float getGainKpRef(const ArmJointID &_jointId, int &result){
            return getGainKpRef(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the Kp reference.
         * Use getGainKpRef(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getGainKpRef(const ArmJointID &_jointId){
            int dummy;
            return getGainKpRef(_jointId, dummy);
        }

         /**
         * @brief Returns the current damping gain (Kd) reference for the specified arm joint.
         *
         * Retrieves the Kd setpoint in Nm*s/rad.
         * The returned value is valid only when result == 1.
         *
         ...
         * @ingroup ArmJointControlAPI
         */
        float getGainKdRef(const ArmJointID &_jointId, int &result){
            return getGainKdRef(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the Kd reference.
         * Use getGainKdRef(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getGainKdRef(const ArmJointID &_jointId){
            int dummy;
            return getGainKdRef(_jointId, dummy);
        }

        /**
         * @brief Return the current motion owner (process ID) of the specified arm joint.
         *
         * In a multi-process control architecture, each joint can only be controlled by a single process at a time —
         * the motion owner. This function returns the process ID currently owning the specified arm joint.
         *
         * @param _jointId The arm joint ID (valid range: 0 to 6).
         * @param joint_processID_ Reference variable to receive the motion owner's process ID.
         *
         ...
         * @ingroup ArmJointControlAPI
         */
        int getMotionOwner(const ArmJointID _jointId, int &joint_processID_){
            return getMotionOwner(static_cast<int>(_jointId), joint_processID_);
        }

        /**
         * @brief Returns the motor temperature of the specified arm joint.
         *
         * Retrieves the current temperature of the joint's motor (in degrees Celsius).
         *
         * @param _jointId The arm joint ID (valid range: 0 to 6).
         ...
         * @ingroup ArmJointControlAPI
         */
        int getMotorTemperature(const ArmJointID _jointId, int &result){
            return getMotorTemperature(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the motor temperature.
         * Use getMotorTemperature(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        int getMotorTemperature(const ArmJointID _jointId){
            int dummy;
            return getMotorTemperature(static_cast<int>(_jointId), dummy);
        }

        /**
         * @brief Returns the board temperature of the specified arm joint.
         *
         * Retrieves the current temperature of the joint's control board (in degrees Celsius).
         *
         * @param _jointId The arm joint ID (valid range: 0 to 6).
         ...
         * @ingroup ArmJointControlAPI
         */
        int getBoardTemperature(const ArmJointID _jointId, int &result){
            return getBoardTemperature(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the board temperature.
         * Use getBoardTemperature(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        int getBoardTemperature(const ArmJointID _jointId){
            int dummy;
            return getBoardTemperature(static_cast<int>(_jointId), dummy);
        }

        /**
         * @brief Returns one motor status flag/value for the specified arm joint.
         *
         * Reads a low-level motor status field for a given arm joint. The returned value is typically 0/1 for boolean flags.
         *
         * Status mapping (see mSTATs):
         * - CAN   → communication link state (connect)
         * - FET   → power stage enable
         ...
         * @ingroup ArmJointControlAPI
         */
        int getMotorStatus(const int &_jointId, const Joint::mSTATs &_req_stat, int &result);
         ...
         * @ingroup ArmJointControlAPI
         */
        int getMotorStatus(const int &_jointId, const Joint::mSTATs &_req_stat){
            int dummy;
            return getMotorStatus(_jointId, _req_stat, dummy);
        }
        ...
        /**
         * @brief Overload using ArmJointID.
         * @see getMotorStatus(const int&, const Joint::mSTATs&, int&)
         * @ingroup ArmJointControlAPI
         */
        int getMotorStatus(const ArmJointID _jointId, const Joint::mSTATs &_req_stat, int &result){
            return getMotorStatus(static_cast<int>(_jointId), _req_stat, result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the status value.
         * @see getMotorStatus(const ArmJointID, const Joint::mSTATs&, int&)
         * @ingroup ArmJointControlAPI
         */
        int getMotorStatus(const ArmJointID _jointId, const Joint::mSTATs &_req_stat){
            int dummy;
            return getMotorStatus(static_cast<int>(_jointId), _req_stat, dummy);
        }

        /**
         * @brief Returns the raw motor position (shaft encoder) in radians.
         *
         * Retrieves the motor-side position (before transmission) for the specified arm joint.
         * The returned value is valid only when result == 1.
         *
         * @param _jointId Arm joint ID (valid: 0–6).
         * @param result Set to 1 on success, -1 if data is unavailable, -2 if joint ID is out of range.
         ...
         * @ingroup ArmJointControlAPI
         */
        float getMotorPos(const ArmJointID _jointId, int &result){
            return getMotorPos(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the motor position.
         * Use getMotorPos(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getMotorPos(const ArmJointID _jointId){
            int dummy;
            return getMotorPos(static_cast<int>(_jointId), dummy);
        }
         ...
         * @brief Returns the raw motor angular velocity in radians per second.
         *
         * Retrieves the motor-side angular velocity for the specified arm joint.
         * The returned value is valid only when result == 1.
         *
         * @param _jointId Arm joint ID (valid: 0–6).
         * @param result Set to 1 on success, -1 if data is unavailable, -2 if joint ID is out of range.
         ...
         * @ingroup ArmJointControlAPI
         */
        float getMotorVel(const ArmJointID _jointId, int &result){
            return getMotorVel(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the motor angular velocity.
         * Use getMotorVel(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getMotorVel(const ArmJointID _jointId){
            int dummy;
            return getMotorVel(static_cast<int>(_jointId), dummy);
        }
         ...
         * @brief Returns the motor current in amperes.
         *
         * Retrieves the instantaneous motor current for the specified arm joint.
         * The returned value is valid only when result == 1.
         *
         * @param _jointId Arm joint ID (valid: 0–6).
         * @param result Set to 1 on success, -1 if data is unavailable, -2 if joint ID is out of range.
         ...
         * @ingroup ArmJointControlAPI
         */
        float getMotorCur(const ArmJointID _jointId, int &result){
            return getMotorCur(static_cast<int>(_jointId), result);
        }

        /**
         * @brief Convenience overload using ArmJointID that returns only the motor current.
         * Use getMotorCur(const ArmJointID, int&) to check the result code.
         * @ingroup ArmJointControlAPI
         */
        float getMotorCur(const ArmJointID _jointId){
            int dummy;
            return getMotorCur(static_cast<int>(_jointId), dummy);
        }
    ...
    struct Simulator {
        ...
        /**
         * @brief Returns the position of the specified arm joint.
         *
         * Retrieves the current position of the specified arm joint in radians.
         *
         * @param _jointId The arm joint ID (valid range: 0 to 6).
         * @param joint_pos_ Reference to store the joint position (in radians).
         * @return 1 on success.
         * @ingroup SimulatorAPI
         */
        int getArmJointPos(const int &_jointId, float &joint_pos_);

        /**
         * @brief Returns the velocity of the specified arm joint.
         *
         * Retrieves the current velocity of the specified arm joint in radians per second.
         *
         * @param _jointId The arm joint ID (valid range: 0 to 6).
         * @param joint_vel_ Reference to store the joint velocity (in radians per second).
         * @return 1 on success.
         * @ingroup SimulatorAPI
         */
        int getArmJointVel(const int &_jointId, float &joint_vel_);

        /**
         * @brief Returns the torque of the specified arm joint.
         *
         * Retrieves the current torque of the specified arm joint in Newton-meters.
         *
         * @param _jointId The arm joint ID (valid range: 0 to 6).
         * @param joint_torque_ Reference to store the joint torque (in Newton-meters).
         * @return 1 on success.
         * @ingroup SimulatorAPI
         */
        int getArmJointTorque(const int &_jointId, float &joint_torque_);
    ...
    struct Parameters {
        ...
        Parameters(RBQ_API *parent) : m_parent(parent) {}
        ...
        double actuatorInertia(const int &joint_id);

        double jointTorqueLimit(const int &joint_id);

        double currnetSensorR(const int &joint_id);

        double jointUpperLimit(const int &joint_id);

        double jointLowerLimit(const int &joint_id);

        double defaultFootHeight();

        double defaultBodyHeight();

        Eigen::Vector3d dockingOffset();

        int maxMarkerReqeustCount();

        int maxDockingFailCount();

        Eigen::Vector3d payloadCoM();

          ...
          try {
            ...
            if (policy.error() == rbq_sdk::Policy::ERROR_NONE && policy.loaded() &&
              ...
              if (params.name == "rbq10") {
                for (int i = 0; i < 3; i++) obs.push_back(gyro[i] * params.obs_ang_vel_scale);
                Eigen::Vector3f proj_grav = quat.inverse() * Eigen::Vector3f(0.0f, 0.0f, -1.0f);
                for (int i = 0; i < 3; i++) obs.push_back(proj_grav[i]);
                static float commands_scale[] = {params.obs_lin_vel_scale, params.obs_lin_vel_scale,
                                                 params.obs_ang_vel_scale};
                for (int i = 0; i < 3; i++) obs.push_back(command[i] * commands_scale[i]);
                for (int i = 0; i < jointSize; i++)
                  obs.push_back((pos[i] - params.default_joint_angles[i]) * params.obs_dof_pos_scale);
                for (int i = 0; i < jointSize; i++) obs.push_back(vel[i] * params.obs_dof_vel_scale);
                ...
                for (int i = 0; i < jointSize; i++)
                  obs.push_back((dof_pos_0[i] - params.default_joint_angles[i]) * params.obs_dof_pos_scale);
                for (int i = 0; i < jointSize; i++)
                  obs.push_back((dof_pos_2[i] - params.default_joint_angles[i]) * params.obs_dof_pos_scale);
                for (int i = 0; i < jointSize; i++)
                  obs.push_back((dof_pos_4[i] - params.default_joint_angles[i]) * params.obs_dof_pos_scale);
                for (int i = 0; i < jointSize; i++)
                  obs.push_back((dof_pos_6[i] - params.default_joint_angles[i]) * params.obs_dof_pos_scale);
...
void goToMotionReady() {
  ...
  bool isGrounded = (pitchAngles[0] > 60 * kD2R && pitchAngles[1] > 60 * kD2R && pitchAngles[2] > 60 * kD2R &&
                     pitchAngles[3] > 60 * kD2R);
  ...
  if (isGrounded) {
    for (int idx = 0; idx < 12; idx++) {
      g_jointControl.setMove(idx, jointTable.ready[idx] * kD2R, motionTime,
                             rbq_sdk::JointControl::MoveCommandMode::Absolute);
    }
    usleep((motionTime + 100) * 1000);
  }
  g_currentTask = TaskState::Idle;
...
void goToMotionGround() {
  ...
  const auto& jointTable = rbq_sdk::JointTable();
  for (int idx = 0; idx < 12; idx++) {
    g_jointControl.setMove(idx, jointTable.folding[idx] * kD2R, motionTime,
                           rbq_sdk::JointControl::MoveCommandMode::Absolute);
  }
  usleep((motionTime + 100) * 1000);
  for (int idx = 0; idx < 12; idx++) {
    g_jointControl.setMove(idx, jointTable.ground[idx] * kD2R, motionTime,
                           rbq_sdk::JointControl::MoveCommandMode::Absolute);
  }
  usleep((motionTime + 100) * 1000);
  g_currentTask = TaskState::Idle;
